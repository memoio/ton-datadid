...
export default function Game () {
    ...
    const gameClick = (pos) => {
        ...
        if ( clickCount < 5000 && timeLeft > 0 ) {
            ...
            pos == 'left'? newElement.className = "w6u9j6 absolute grobold text-xl text-[#FFFFFF] left-0": newElement.className = "w6u9j6 absolute grobold text-xl text-[#FBCC18] right-0"; // Add a class for styling            
    ...
    return (
        <>
            ...
            <div ref={ gameB } className="fixed inset-x-8 bottom-8 p-4 rounded-xl shadow-md shadow-dao-green/50 flex gap-4 items-center justify-between z-10">
                <div className="flex gap-4">
                    <Image src={"Clip path group-clock.svg"} width={35.19} height={43} alt="" />
                    <div className="flex flex-col min-w-1/2">
                        <p className="">Time Remaining</p> 
                        <p className="text-dao-green text-sm uppercase">{ timeLeft }s</p> 
                    </div>
                </div>
                <button onClick={ reset } type="reset"><Image src={"Group 34604.svg"} className="size-8" width={36} height={36} alt="" /></button>
            </div>

            <div ref={ gameC } className="min-h-svh min-w-screen flex justify-center items-center relative">
                <div className="relative w-full">
                    ...
                    <div className="relative w-fit mx-auto">
                        <Image src={"/Group 34528.png"} className="mx-auto" width={215} height={215} alt="" />
                        <Image src={"/Group 34531.svg"} className="absolute scale-75 right-12 bottom-8" width={49} height={53.03} alt="" />
                    </div>
                    ...
                    <div className="w-full px-12 flex flex-col items-end gap-2 mt-12">
                        ...
                        <div className="flex gap-2">
                            ...
                            <p className="grobold text-[#FBCC18] relative top-0.5">{ clickCount }/<span className="grobold text-white">5000</span></p>
                        </div>
                    </div>
                </div>
            ...
            <div ref={ gameO } className="min-h-svh min-w-screen hidden flex-col justify-center items-center p-8 fixed z-10 inset-0">
                { clickCount == 5000 && timeLeft <= 0? <Image src={"/Group 34536.png"} className="fixed inset-0 w-screen h-screen aspect-auto object-cover opacity-10" width={307.24} height={463} alt="" />:
                <Image src={"/embarrassed 1.png"} className="fixed inset-0 w-screen h-screen aspect-auto object-cover opacity-10" width={64} height={64} alt="" />
                }
                ...
                <div className="flex flex-col gap-4 bg-[#02402791] w-full p-8 rounded-xl text-center relative z-10 border-b-4 border-solid border-dao-green">
                    ...
                    { clickCount == 5000 && timeLeft <= 0? <p className="">Great job! You just earned { clickCount } points!</p>: 
                    <p className="">You only earned { clickCount } points!</p> }
                    <button onClick={ () => router.reload() } className="border-y-2 border-solid border-dao-green rounded-full px-6 w-fit py-3 mx-auto flex gap-2 items-center"><Image src={"tabler_reload.svg"} width={24} height={24} alt="" />Play Again</button>
                </div>
            </div>
        </>
    )
